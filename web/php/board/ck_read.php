...
<script type="text/javascript" src="/a/js/jquery-3.2.1.min.js"></script>
<script type="text/javascript" src="/a/js/jquery-ui.js"></script>
	 ...
	 <?php
	 	...
	 	if(isset($_POST['pw_chk'])) //만약 pw_chk POST값이 있다면
	 	{
			...
			if(password_verify($pwk,$bpw)) //다시 if문으로 DB의 pw와 입력하여 받아온 bpw와 값이 같은지 비교를 하고
			{
				$pwk == $bpw;
			?>
				<script type="text/javascript">location.replace("/a/read.php?idx=<?php echo $board["idx"]; ?>");</script><!-- pwk와 bpw값이 같으면 read.php로 보내고 -->
			<?php 
			}else{ ?>
